...
function buildInstruction(instruction: string, sceneState: any, assetState: any) {
	const SYSTEM = `
You are an AI copilot embedded into a video editor. Users can chat with you to create and edit videos, and your job is to create a JSON video representation that is converted into an actual video. You can also create assets, which are based on files, through function calls. The video format is 1920x1080. Note that the background is white by default, so if you add text and don't specify a background, you will have to specify a text color.
...
async function sendInstructionToGPTInner(instruction: string, state: Store) {
	...
	const jsonString = findOutermostJSON(response);
	if (!jsonString) {
		throw new Error('no valid json found in response');
	}
...
async function generateImage({
	prompt,
	dimensions,
}: AiImageAsset['instructions']): Promise<AiImageAsset> {
	const jobId = crypto.randomUUID();
	const filePath = `${jobId}-image.png`;

	await fluxGenerate(prompt, dimensions, `./public/${filePath}`);

	return {
		type: 'ai_image',
		instructions: {prompt, dimensions},
		properties: {filePath},
	};
}
...
export async function dalleGenerate(
	prompt: string,
	dimensions: '1792x1024' | '1024x1792',
	savePath: string,
) {
...
async function fluxGenerate(
	...
	const aspectRatio = dimensions === '1024x1792' ? '9:16' : '16:9';
	...
	try {
		...
		if (Array.isArray(output) && output.length > 0) {
			...
			if (!(imageStream instanceof ReadableStream)) {
				throw new Error(`Invalid image stream: ${JSON.stringify(imageStream)}`);
			}
			...
			throw new Error('No image stream returned from Flux Schnell');
		}
	} catch (error) {
		console.error('Error in fluxGenerate:', error);
		throw error;
	}
}
